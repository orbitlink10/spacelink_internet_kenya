...
<div class="max-w-6xl mx-auto px-6 py-10">
    ...
    <form method="GET" class="grid gap-4 md:grid-cols-4 mb-6 bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
        ...
        <button class="md:col-span-2 px-4 py-2 bg-blue-600 text-white rounded">Filter</button>
    </form>
        ...
        @forelse($products as $product)
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-col">
                ...
                <div class="mt-auto flex gap-2 pt-3">
                    <a href="{{ route('products.show', $product->slug) }}" class="px-3 py-2 rounded border border-slate-200 text-slate-800">View</a>
                    @if($product->inStock())
                        <form method="POST" action="{{ route('cart.add', $product->id) }}" class="flex-1 text-right">
                            @csrf
                            <input type="hidden" name="quantity" value="1">
                            <button class="px-3 py-2 rounded bg-blue-600 text-white w-full">Add to cart</button>
                        </form>
                    @endif
                </div>
            </div>
